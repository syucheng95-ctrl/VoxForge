...
CT 图像 + 自由文本 prompt
  -> Stage 0：文本路由 + 解剖裁剪
  -> Stage 0.5：多专家候选框生成
  -> Stage 1：VoxTell 验证 + proposal 筛选
  -> Stage 2：STU-Net ROI 推理
...
| Stage 1 | `pipeline/stage1_verify.py`, `pipeline/build_s1_vp.py` | 用 VoxTell 验证候选框，筛选 proposal |
| Stage 2 | `pipeline/stage2_rois.py`, `pipeline/stage2_eval.py`, `pipeline/build_s2_vp.py` | 构建 ROI，运行 STU-Net 推理 |
| Gate | `pipeline/build_gate_table.py`, `pipeline/apply_gate.py` | 构建推理特征表，预测每个 finding 使用 S1 还是 S2 |
